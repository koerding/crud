...
\textbf{Crucial framing first.} The diminishing-returns claim below applies only to a single, narrow purpose: distinguishing a small adjusted association from typical background dependence at the level of magnitude alone. Large cohorts (UK Biobank, All of Us, Million Veteran Program) remain essential for everything else---detecting subgroup effects, interactions, rare outcomes, time-to-event endpoints with low censoring, dose-response shape, replication, and improving precision on already-distinctive effects. None of the discussion below should be read as ``stop running large cohorts.''

The crud-aware framework yields a concrete insight about sample size for the magnitude-separation purpose specifically. The $z$-test denominator $\sqrt{\sigma_{\mathrm{crud}}^2 + 1/(n-1)}$ reveals a natural crossover. When $n$ is small ($n \ll 1/\sigma_{\mathrm{crud}}^2$), sampling noise dominates; larger samples genuinely increase power to detect effects. But once $n$ is large enough that the sampling noise $1/(n-1)$ is small relative to the crud variance $\sigma_{\mathrm{crud}}^2$, the denominator is approximately $\sigma_{\mathrm{crud}}$ regardless of $n$: the bottleneck is no longer imprecise estimation but the fact that background correlations are themselves nonzero. Additional observations measure the crud more precisely but do not help separate signal from background.
Because $\sigma_{\mathrm{crud}} \approx 0.02$--$0.14$ across the domains in Table~\ref{tab:crud-scale} (at $K=10$), the crossover occurs at roughly $n \approx 50$--$2{,}500$. Beyond this point, classical $p$-values continue to shrink---explaining why massive studies routinely report ``significant'' but tiny effects---while the crud-aware $p$-value plateaus. For the magnitude-separation purpose, there are sharply diminishing returns to collecting more than a few thousand observations beyond the crossover; for the many other purposes large cohorts are designed to serve, the diminishing-returns claim does not apply.
...
\midrule
\multicolumn{7}{l}{\emph{Crud-aware (95th percentile), $K$-sensitivity against $L=10$ latent factors:}} \\
\phantom{XX}$K=0$ (no adjustment)     & $\phantom{0}995$ & $\phantom{0}9$ & $\phantom{0}986$ & $0.009$ & $0.19$ & $0.018$ \\
\phantom{XX}$K=5$ (under-adjusted)    & $\phantom{0}995$ & $34$ & $\phantom{0}961$ & $0.034$ & $0.68$ & $0.065$ \\
\phantom{XX}$K=10$ (matched to $L$)   & $\phantom{0}995$ & $49$ & $\phantom{0}946$ & $0.049$ & $0.98$ & $0.094$ \\
\phantom{XX}$K=15$ (over-adjusted)    & $\phantom{0}995$ & $39$ & $\phantom{0}956$ & $0.039$ & $0.78$ & $0.075$ \\
\phantom{XX}$K=20$ (over-adjusted)    & $\phantom{0}995$ & $28$ & $\phantom{0}967$ & $0.028$ & $0.56$ & $0.054$ \\
\midrule
...
\paragraph{$K$-sensitivity.} The DGP has $L=10$ latent factors. At $K=10$ the PC adjustment exactly matches latent dimensionality (the most favorable regime). The five rows of the middle block in Table~\ref{tab:synthetic-benchmark} show what happens at $K\in\{0,5,10,15,20\}$: F1 peaks at $K=L$, but the crud-aware test still beats every FDR-based competitor (Efron+BH F1$=0.036$) at $K=5$ (F1$=0.065$) and $K=15$ (F1$=0.075$), and remains competitive even at $K=20$ (F1$=0.054$, double Efron's). The advantage degrades smoothly with mismatch rather than collapsing, so the user does not need to know $L$ exactly---a reasonably large $K$ in the vicinity of the true latent dimensionality preserves most of the benefit.

\paragraph{Other caveats.} The DGP is stylized, with fixed loadings, Gaussian noise, and edge-strengths chosen to make the comparison readable. We do not claim the crud-aware test dominates alternatives in all regimes; if confounding is sparse and concentrated rather than broadly distributed, BH-FDR will perform much better. The takeaway is narrower: in regimes where the residual correlation distribution is broad and the user's adjustment dimension is reasonably close to the latent confounding (the regime the empirical sections of this paper document across nine real datasets), the crud-aware diagnostic provides a useful magnitude check that classical and FDR-based methods do not.
...
Three explicit warnings about the catalog: (i)~\emph{Domain matches are rough.} Each row pairs an application domain with a single reference, but $\sigma_K$ on a true same-pipeline reference can plausibly differ from the listed value by a factor of $1.5$--$2$ in either direction. For instance, the listed NHANES $\sigma_K=0.093$ is for a general health-survey cohort with $p=165$; a cancer-registry-linked cohort or a controlled-access biobank with hundreds or thousands of features will have a different $\sigma_K$ that we cannot estimate from these nine datasets. (ii)~\emph{Always run a sensitivity sweep.} For any reported result, recompute the conclusion across at least two plausible $\sigma_K$ values---the listed reference and one neighboring entry from the table. If the conclusion changes, the magnitude verdict is not robust and should be reported with that caveat rather than a single $p_{\mathrm{crud}}$. (iii)~\emph{Avoid mismatched designs.} Cohort, case-control, registry-linked, and convenience-sample studies have systematically different $\sigma_K$; matching the design type to the reference is more important than matching the topic (cf.\ caveat (vii) in Appendix~\ref{app:scale-bridge}).
...
\paragraph{A small effect that does not.} Liu et al.~\citep{liu2022} report a nominally significant association between folate and mortality in adults with T2D (folate quartile 1 vs 2 HR 1.17, 95\% CI 1.01--1.37) in an NHANES-derived cohort of adults with type~2 diabetes ($n \approx 8{,}000$). Converting from the reported confidence interval via $z = \log(\mathrm{HR})/\mathrm{SE}(\log\mathrm{HR})$ and $r \approx z/\sqrt{n}$ gives $r \approx 0.02$, $p_{\mathrm{classical}} \approx 0.04$.
