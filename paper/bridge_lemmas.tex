...
Heavy tails put more mass in the upper percentiles than a Gaussian formula predicts, so a parametric crud-aware $p$-value will tend to understate how typical a moderately large $|r|$ actually is---i.e., the parametric test is anti-conservative in the tail in those datasets.
We have no general analytic correction for this; the recommendation is to use the full empirical null (Section~\ref{app:crud-aware-pvalues}) whenever the reference dataset is large enough, and to treat the parametric formula as an approximate tool when only summary statistics are available.
