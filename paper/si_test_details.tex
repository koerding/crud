...
\paragraph{Results.} Table~\ref{tab:synthetic-benchmark} reports flagged count, true positives, false positives, precision, recall, and F1 for one run (seed fixed for reproducibility; reproducer in \texttt{scripts/synthetic\_benchmark.py}).

\begin{table}[h]
\centering
\caption{Method comparison on the synthetic DGP. Truth: $E=50$ direct edges out of $M=19{,}900$ off-diagonal pairs (prior $P(\mathrm{edge})=0.0025$). Empirical $\sigma_0=0.066$; $\sigma_{K=10}=0.029$.}
\label{tab:synthetic-benchmark}
\begin{tabular}{lcccccc}
\toprule
Method & Flagged & TP & FP & Precision & Recall & F1 \\
\midrule
Classical $|r|>1.96/\sqrt{n-1}$       & $4{,}478$ & $48$ & $4{,}430$ & $0.011$ & $0.96$ & $0.02$ \\
Bonferroni ($\alpha/M$)               & $1{,}851$ & $32$ & $1{,}819$ & $0.017$ & $0.64$ & $0.03$ \\
BH-FDR (theoretical null, $q=0.05$)   & $3{,}277$ & $48$ & $3{,}229$ & $0.015$ & $0.96$ & $0.03$ \\
Efron empirical null + BH ($q=0.05$)  & $2{,}412$ & $43$ & $2{,}369$ & $0.018$ & $0.86$ & $0.04$ \\
Crud-aware, 95th pct, $K=0$           & $\phantom{0}995$ & $\phantom{0}9$ & $\phantom{0}986$ & $0.009$ & $0.18$ & $0.02$ \\
Crud-aware, 95th pct, $K=10$          & $\phantom{0}995$ & $48$ & $\phantom{0}947$ & $0.048$ & $0.96$ & $0.09$ \\
Crud-aware, 99th pct, $K=10$          & $\phantom{0}199$ & $40$ & $\phantom{0}159$ & $0.20$  & $0.80$ & $0.32$ \\
\bottomrule
\end{tabular}
\end{table}
...
(i) \emph{Classical multiple-testing methods fail to control false discoveries when confounding is structural.} BH-FDR against the theoretical $\mathcal{N}(0,1)$ null is supposed to control the false discovery rate at $q=0.05$, but here it returns $3{,}229$ false discoveries against $48$ true ones (FDR $\approx 0.985$, far above $0.05$). The reason is well-known: BH assumes the $p$-values under the null are independent or weakly dependent, which fails badly under shared latent factors. Bonferroni is more conservative on flagged count but still recovers more false positives than true ones. Efron's empirical null \citep{efron2004} addresses part of this by widening the null distribution ($\sigma_0=1.35$ in this run, vs.\ the theoretical $\sigma_0=1$), which reduces flagged count modestly ($2{,}412$ vs.\ $3{,}277$) and yields slightly better F1, but it still fails to control FDR because the empirical null only recalibrates the marginal scale, not the structure that makes background pairs correlated. Efron's empirical null is the closest standard competitor to the crud-aware approach; it is improved upon by orders of magnitude when the structural confounding is removed via PC adjustment.
