...
\paragraph{Caveats.} The DGP is stylized, with fixed loadings, Gaussian noise, and edge-strengths chosen to make the comparison readable. The DGP also has $L=10$ latent factors and we adjust at $K=10$, so the PC adjustment exactly matches the latent dimensionality---the most favorable regime for the crud-aware test. When $K$ is mismatched to $L$ (under-adjusted: residual confounding remains; over-adjusted: signal absorbed into removed PCs), the crud-aware advantage shrinks and the comparison with BH/Efron is less one-sided. We do not claim the crud-aware test dominates alternatives in all regimes; if confounding is sparse and concentrated rather than broadly distributed, BH-FDR will perform much better. The takeaway is narrower: in regimes where the residual correlation distribution is broad and the user's adjustment dimension is reasonably matched to the latent confounding (the regime the empirical sections of this paper document across nine real datasets), the crud-aware diagnostic provides a useful magnitude check that classical and FDR-based methods do not.
