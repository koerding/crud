...
In practice, $p_{\mathrm{crud}}(i,j)$ is approximated by sampling $M$ reference pairs uniformly from all $\binom{p}{2}$ pairs, computing $T_{A_mB_m}$ for each, and taking the fraction (plus one, for the standard add-one correction) whose absolute value exceeds $|T_{ij}|$. For a uniformly random target pair, $p_{\mathrm{crud}}$ is (super-)uniform by a standard exchangeability argument. For a target pair selected non-randomly, the test continues to control Type-I error \emph{only under the null} that the pair's true association is exchangeable with the domain background. Selection by prior hypothesis or pilot data may violate this null---e.g., pairs flagged by an earlier scan of the same data are no longer exchangeable with random pairs---so for such targets the crud-aware percentile is interpreted relative to the background but does not by itself adjust for selection.
