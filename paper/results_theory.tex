...
\]
Intuitively, the numerator measures how unevenly the remaining variance is distributed across components (the more concentrated it is, the wider the residual correlations), while the denominator measures the total remaining variance (which sets the scale). The result requires that individual PCs spread their influence broadly across features rather than loading on just a few---a condition called ``eigenvector delocalization.'' The empirical audit (Appendix~\ref{app:assumption-audit}, Test~2) finds that this condition is met only approximately: eigenvector coherence is $3.8$--$11.2\times$ the Haar-uniform ideal across our nine datasets, with the largest deviations in low-$p$ data. The resulting bias on this spectrum-only formula is conservative for the crud-scale claim (Section~\ref{app:approx-robust}): localization tends to make the formula overestimate the typical residual correlation, so the qualitative message---that low-rank adjustment shrinks the crud scale only modestly---is not threatened by the partial violation. (The audit also reports heavy-tailed residuals, which bias the Theorem~\ref{thm:assoc-only} error formula in the optimistic direction; we discuss that separately.) The spectral prediction tracks the empirical residual SD across values of $K$ in all nine datasets.
...
\]
where $\Phi$ is the standard normal CDF. In particular, if $|\mu|$ is at most a constant multiple of $\sigma_K$, the error is bounded below by a constant bounded away from zero, regardless of sample size. This limitation persists as $n\to\infty$ within our model: under the delocalization assumption and fixed $K$, $\sigma_K$ is a population quantity that more samples cannot reduce. Whether this model-level quantity matches the conceptually natural notion of ``background dependence'' in a given domain depends on whether the chosen adjustment captures that domain's confounding structure. Put plainly: if the causal signal (in adjusted-correlation units) is not larger than the crud scale, then no correlation-only rule can reliably separate causal from noncausal pairs.
